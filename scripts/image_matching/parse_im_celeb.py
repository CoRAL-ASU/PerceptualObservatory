...
# ---------------------- LOAD CELEB CACHE ----------------------
with open("./results/celeb_cache.json", "r", encoding="utf-8") as f:
...
# ---------------------- MAIN PARSER ----------------------
def parse_folder(folder: str, dataset="celeb"):
    parsed = {}
    ...
    for fname in files:
        ...
        for ex in data:
            ...
            # --- Resolve celeb info from cache ---
            celeb_info = CELEB_CACHE.get(test_key, {})
            celeb_label = celeb_info.get("label", "unknown")
            parsed_label = celeb_info.get("parsed_label", "unknown")
            gender = celeb_info.get("gender", "unknown")
            ...
            if split == "gt":
                ...
                entry[split] = {
                    ...
                    "option_ids": option_ids,
                    "answer_idx": answer_idx,
                    "pred_idx": pred_idx,          # mapped A=0, B=1, C=2, D=3
                    "pred_answer": pred_answer,
                    "model": ex.get("model"),
                }
...
def main():
    ...
    out_path = os.path.join(output_folder, "all_parsed_celeb_im.json")
