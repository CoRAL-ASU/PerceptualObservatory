...
def build_rich_summary(results):
    """
    Add GT/AUG/ILLUS for full dataset AND for male/female separately.
    """
    summary = {}
    ...
    for model, model_stats in results.items():
        ...
        if aug_splits:
            entry["aug"] = {
                "accuracy": float(np.mean([v["example_accuracy"] for v in aug_splits.values()])),
                "pos_gap": float(np.mean([v["pos_gap"] for v in aug_splits.values()])),
            }
        ...
        if illus_splits:
            entry["illus"] = {
                "accuracy": float(np.mean([v["example_accuracy"] for v in illus_splits.values()])),
                "pos_gap": float(np.mean([v["pos_gap"] for v in illus_splits.values()])),
            }
        ...
        for gender in ["male", "female"]:
            if gender not in model_stats:
                continue

            g_stats = model_stats[gender]
            g_entry = {}
            ...
            if "gt" in g_stats:
                gt = g_stats["gt"]
                g_entry["gt"] = {
                    "accuracy": gt.get("example_accuracy", 0.0),
                    "pos_gap": gt.get("pos_gap", 0.0),
                }

            # AUG
            aug_splits = {k: v for k, v in g_stats.items() if k.startswith("aug")}
            if aug_splits:
                g_entry["aug"] = {
                    "accuracy": float(np.mean([v["example_accuracy"] for v in aug_splits.values()])),
                    "pos_gap": float(np.mean([v["pos_gap"] for v in aug_splits.values()])),
            ...
            # ILLUS
            illus_splits = {k: v for k, v in g_stats.items() if k.startswith("illus")}
            if illus_splits:
                g_entry["illus"] = {
                    "accuracy": float(np.mean([v["example_accuracy"] for v in illus_splits.values()])),
                    "pos_gap": float(np.mean([v["pos_gap"] for v in illus_splits.values()])),
...
def shorten_model_name(name: str) -> str:
    ...
    # --- Thinking models keep -T ---
    if re.search(r"thinking$", name, flags=re.IGNORECASE):
        name = re.sub(r"-thinking$", "-T", name, flags=re.IGNORECASE)
        return name

    # --- Cleanup: drop other suffixes (instruct, it, hf) ---
    name = re.sub(r"-(instruct|it|hf|HF|Instruct)$", "", name, flags=re.IGNORECASE)

    return name


def plot_accuracy_vs_gap(points_male, points_female, title, filename):
    ...
    # Plot male (circle) + female (square) + connecting line
    for model, (gx, gy) in points_male.items():
        family = get_family(model)
        base_color = FAMILY_COLORS.get(family, "gray")
        ax.scatter(gx, gy, color=base_color, marker="o", s=70,
                   edgecolor="black", linewidth=0.6, label=f"{family} (Male)")
        # ax.text(gx+0.01, gy+0.01, shorten_model_name(model), fontsize=8)
        if model in points_female:
            fx, fy = points_female[model]
            ax.plot([gx, fx], [gy, fy], color=base_color, linestyle="-", linewidth=1.0)

    for model, (gx, gy) in points_female.items():
        family = get_family(model)
        base_color = FAMILY_COLORS.get(family, "gray")
        ax.scatter(gx, gy, color=base_color, marker="s", s=70,
                   edgecolor="black", linewidth=0.6, label=f"{family} (Female)")
...
def main():
    ...
    ap.add_argument("--parsed_json", default="./results/gpg/celeb/all_parsed_celeb_gpc.json")
    ap.add_argument("--out", default="./results/gpg/celeb/avg_performance_celeb_gpc.json")
    args=ap.parse_args()
    ...
    for model,recs in models.items():
        ...
        for gender in ["male", "female"]:
            g_recs = [r for r in recs if r.get("gender") == gender]
            if g_recs:
                results[model][gender] = compute_model_stats(g_recs)
    ...
    for split in ["gt", "aug", "illus"]:
        ...
        for model, vals in rich_summary.items():
            ...
            # MALE
            if "male" in vals and split in vals["male"]:
                gap = vals["male"][split]["pos_gap"]
                acc = vals["male"][split]["accuracy"]
                pts_male[model] = (gap, acc)

            # FEMALE
            if "female" in vals and split in vals["female"]:
                gap = vals["female"][split]["pos_gap"]
                acc = vals["female"][split]["accuracy"]
                pts_female[model] = (gap, acc)

        if pts_male or pts_female:
            fig_path = args.out.replace(".json", f"_{split}.png")
            plot_accuracy_vs_gap(
                pts_male,
                pts_female,
                f"{split.upper()}: Accuracy vs Gap",
                fig_path
            )
