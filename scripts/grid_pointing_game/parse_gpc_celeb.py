...
def parse_model_name(source_file: str) -> str:
    ...
    name = base.replace("_gpg-celeb.jsonl", "").replace("_gpg-celeb.json", "")
    return name
...
def parse_record(
    rec: Dict[str, Any],
    source_file: str,
    celeb_cache: Dict[str, Dict[str, str]]
) -> Dict[str, Any]:
    ...
    # ---- lookup from celeb_cache ----
    cache_entry = celeb_cache.get(support, {})
    raw_label = cache_entry.get("label", "")
    parsed_label = cache_entry.get("parsed_label", raw_label)
    gender = cache_entry.get("gender", "unknown")
...
def parse_folder(folder: str, celeb_cache: Dict[str, Dict[str, str]]) -> List[Dict[str, Any]]:
    files = glob.glob(os.path.join(folder, "*.jsonl"))
    parsed: List[Dict[str, Any]] = []
    for f in files:
        for rec in load_jsonl(f):
            parsed.append(parse_record(rec, f, celeb_cache))
    return parsed
...
def main():
    ...
    ap.add_argument("--folder", default="./results_gpc/celeb/", help="Folder containing *.jsonl result files")
    ap.add_argument("--out-json", default="./results/all_parsed_celeb_gpc.json")
    ap.add_argument("--celeb-cache", default="/gscratch/tkishore/results/celeb_cache.json",
                    help="Path to celeb_cache.json with label+gender info")
    args = ap.parse_args()

    with open(args.celeb_cache, "r", encoding="utf-8") as f:
        celeb_cache = json.load(f)

    parsed = parse_folder(args.folder, celeb_cache)
    ...
    print(f"🧾 Labels + gender loaded from {args.celeb_cache}")
